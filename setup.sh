...
not_found_error() {
  echo "Unable to determine Flutter version: version=$1 architecture=$2"
}
...
# 🔥 REMOVE CHANNEL (force neutral)
CHANNEL="any"

[ -z "$VERSION" ] && VERSION=any
[ -z "$ARCH" ] && ARCH=x64
[ -z "$CACHE_PATH" ] && CACHE_PATH="$RUNNER_TOOL_CACHE/flutter/:version:-:arch:"
[ -z "$CACHE_KEY" ] && CACHE_KEY="flutter-:os:-:version:-:arch:-:hash:"
[ -z "$PUB_CACHE_KEY" ] && PUB_CACHE_KEY="flutter-pub-:os:-:version:-:arch:-:hash:"
...
# 🔥 Use simple version filtering only (no channel)
VERSION_MANIFEST=$(echo "$RELEASE_MANIFEST" | jq '.releases' | filter_by_arch "$ARCH" | filter_by_version "$VERSION")

case "$VERSION_MANIFEST" in
*null*)
  not_found_error "$VERSION" "$ARCH"
  exit 1
  ;;
esac
...
expand_key() {
  ...
  expanded_key="${1/:version:/$version_version}"
